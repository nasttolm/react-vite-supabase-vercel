...
function formatIngredients(recipe) {
  ...
  // Process ingredients in the known structure from getRecipeById
  if (Array.isArray(recipe.ingredients)) {
    return recipe.ingredients.map(item => {
      // The expected structure is: item.ingredients.name and item.grams
      if (item.ingredients && item.ingredients.name) {
        return {
          name: item.ingredients.name,
          amount: parseFloat(item.grams) || 1,
          unit: 'g',
      ...
      return {
        name: item.name || (item.ingredients && item.ingredients.name) || 'Unknown ingredient',
        amount: parseFloat(item.grams || item.amount) || 1,
        unit: 'g',
        category: 'Ingredients'
      };
    });
    ...
    Object.entries(day).forEach(([categoryId, meals]) => {
      ...
      meals.forEach((meal, mealIndex) => {
        ...
        mealIngredients.forEach((ingredient) => {
          ...
          const ingredientAmount = typeof amount === 'number' ? amount : parseFloat(amount) || 1;
